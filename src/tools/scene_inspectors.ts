...
export function handleReadLight2d(
  ...
  } catch (e: any) { return toolError(ErrorCode.INTERNAL_ERROR, `Error: ${e.message}`); }
}
...
export function handleReadAudioStream(projectRoot: string, args: { path: string }): ToolResult {
  try {
    ...
    if (!fs.existsSync(abs)) return toolError(ErrorCode.FILE_NOT_FOUND, `File not found: ${args.path}`);
